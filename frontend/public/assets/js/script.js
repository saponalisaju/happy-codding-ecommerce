...
function rewriteMirroredAssetUrls() {
  document
    .querySelectorAll("[src], [href], [poster], use")
    .forEach((element) => {
...
document.querySelectorAll(".product").forEach((product) => {
  ...
  const updateIcon = () => {
    if (!decIcon) return;
    decIcon.className = count === 1 ? "ri-delete-bin-line" : "ri-subtract-line";
  };
...
=======================*/
document
  .querySelectorAll(".footer-section .footer-title")
  .forEach((heading) => {
...
document.querySelectorAll(".cart-offcanvas").forEach((cart) => {
  ...
  function checkNoCart(cart) {
    let productListItems = cart.querySelectorAll(
      ".cart-product-box .product-list li",
    );
  ...
  productContents.forEach((pc) => {
    ...
    if (priceBox) {
      let mainPrice = parseFloat(
        priceBox.childNodes[0].textContent.replace("$", ""),
      );
  ...
  productContents.forEach((pc, i) => {
    ...
    if (span) {
      ...
      if (counter)
        counter.querySelector(".count").textContent = qty
          .toString()
          .padStart(2, "0");
    }
  });
...
window.addEventListener("load", function () {
  ...
  const newsletterNoShowCheckbox =
    document.getElementById("newsletter-no-show");
...
window.addEventListener("load", function () {
  ...
  function startExitCountdown(durationInMinutes) {
    ...
    const interval = setInterval(function () {
      ...
      if (document.getElementById("exit-min-1"))
        document.getElementById("exit-min-1").textContent = min1;
      if (document.getElementById("exit-min-2"))
        document.getElementById("exit-min-2").textContent = min2;
      if (document.getElementById("exit-sec-1"))
        document.getElementById("exit-sec-1").textContent = sec1;
      if (document.getElementById("exit-sec-2"))
        document.getElementById("exit-sec-2").textContent = sec2;
...
document.addEventListener("DOMContentLoaded", function () {
  const dropdownLinks = document.querySelectorAll(
    ".navbar-nav .dropdown > .nav-link, .navbar-nav .dropdown .dropdown-menu .nav-item > a, .home-demo-menu .menu-col .menu-header",
  );
    ...
    link.addEventListener("click", function (e) {
      if (window.innerWidth <= 991.98) {
        ...
        const menu = parent.querySelector(
          ":scope > .dropdown-menu, :scope > .mega-menu",
        );
        ...
        if (menu) {
          ...
          for (let sibling of siblings) {
            if (sibling !== parent) {
              ...
              const otherMenu = sibling.querySelector(
                ":scope > .dropdown-menu,:scope > .mega-links, :scope > .mega-menu",
              );
  ...
  const menuHeaders = document.querySelectorAll(
    ".navbar-nav .menu-col .menu-header",
  );
...
document.addEventListener("DOMContentLoaded", function () {
  const dropdownLinks = document.querySelectorAll(
    ".header-6 .navbar-nav .dropdown > .nav-link, .navbar-nav .dropdown .dropdown-menu .nav-item > a, .home-demo-menu .menu-col .menu-header",
  );
  ...
  dropdownLinks.forEach((link) => {
    link.addEventListener("click", function (e) {
      const parent = this.parentElement;
      const menu = parent.querySelector(
        ":scope > .dropdown-menu, :scope > .mega-menu, :scope > .mega-links",
      );
...
document.querySelectorAll(".qty-control").forEach((c) => {
  ...
  c.querySelector(".minus").onclick = () =>
    (i.value = Math.max(min, +i.value - 1));
  c.querySelector(".plus").onclick = () =>
    (i.value = Math.min(max, +i.value + 1));
});
